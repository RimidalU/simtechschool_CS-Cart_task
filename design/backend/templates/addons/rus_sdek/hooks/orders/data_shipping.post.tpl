...
{if $shipment.carrier == "sdek"}
    ...
    {if $data_shipments.$shipment_id}
        ...
        {else}
            <a href="{"orders.sdek_order_status?shipment_id=`$shipment.shipment_id`&order_id=`$order_id`"|fn_url}" class="cm-post"><span>{__("sdek.update_status")}</span></a>
        {/if}
    {/if}
{/if}
<hr />
