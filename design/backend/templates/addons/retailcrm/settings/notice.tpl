...
<div id="collapsable_instructions" class="in collapse">
    <ol>
        ...
        <li>{__("retailcrm.settings.instructions.step_price_list", ["[href_price_list]" => "yml.manage"|fn_url]) nofilter}</li>
        <li>{__("retailcrm.settings.instructions.step_cron_command", ["[cron_command]" => $retailcrm_order_sync_console_cmd]) nofilter}</li>
    </ol>
</div>